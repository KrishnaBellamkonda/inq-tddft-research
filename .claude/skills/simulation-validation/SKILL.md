...
description: Use when proposing benchmarks, defining convergence tests, or writing a validation plan for any INQ ground-state or TDDFT calculation in `ResearchProject/` or `Tutorial/`. Required before launching expensive runs — produces a tiered test menu the user must approve.
---

# Simulation Validation

## 1. Propose the validation menu (before running anything expensive)
...
- Time-step convergence (dt = 0.08, 0.04, 0.02 atomic time — compare spectra)
...
In the handover or `docs/validation/<system>.md`:
...
| Jellium slab/box | Free-electron benchmark | Analytic limits known |

GPU is the default execution path (`inq-run`, no `--cpu`). Only use `--cpu` for the
explicit GPU/CPU consistency check or when no GPU is available.
