...
### Step 0 — baseline committed (6 commits)
`refactor(inqkit)` · `feature(inqkit)` tests · `feature(inqview)` kernels ·
`refactor(inqview)` 4-package split · `docs` · `chore(repo)` rule. Gate GREEN
(86 pass / 5 xfail / 1 xpass baseline).

### Group A — DONE (3/3), gate green after each, committed
- **A1** `plasmon_spectrum.spectrum_3d_binned` (shell-averaged |n_q(ω)|²) +
  `pipeline.frames` VTI loader + analytic shell-peak test. → 87 pass.
- **A2** `pipeline.cod` from-run COD + `cod_offset_vs_inqkit` dx/2 cross-check +
  pure test. → 88 pass.
- **A3** `gs_projected_occupations` t=0 identity test (`_project_full`: O=I→f_i;
  conservation; swap). → 91 pass.

### Group B — DONE (B1, B3); B2 DEFERRED
- **B1** moved `wake.shared_clim` → `visualisation/wake.py`, re-exported in
  pipeline for back-compat, retargeted test_wake. → 91 pass.
- **B3** moved KL `_normalise`/`_kl` → `analysis/kl_divergence.py`, re-exported in
  pipeline, retargeted test. → 91 pass.
- **B2 DEFERRED (not a failure):** `energy_balance` compute is fully embedded in
  `run()` with plotting and has NO test guarding it; the primary functional split
  is already provided by `analysis.energy_components` (IV-M07). A safe split needs
  a test-first supervised session. Logged for phase-3.

### Group C — partial: safe cleanup DONE, back-compat-risky items DEFERRED
- **C5 ✓** purged `.understand-anything/.trash` cruft (gitignored).
- **C6 ✓** CLAUDE.md inqview section → 4-package layout (was stale flat map).
- **C8 ✓** wrote `inq-stack/README` (public API per layer, was empty).
- **C2 ✓ (by state)** `email.py` is ALREADY out of the public API (not exported);
  renaming would break `from inqview.email import send_run_email` in run scripts.
- **DEFERRED (back-compat / surprise cost; phase-3):** C1 `config`/`defaults`
  still used by `plots` (cut needs a plots refactor); C3 `vti.py` still used by
  `paraview`/`defaults`; C4 relocate `report1`/`scripts` (actively used via the
  report-figures skill — high surprise); C7 rename `pipeline.pipeline`→`runner`
  (would break `from inqview.postprocess import pipeline` — conflicts with the
  keep-shim decision). Do C4+C7 in phase-3 alongside migrating run analyse.py.

### Group D — DEFERRED (both), documented findings
- **D1 (Vec3 current/dipole) DEFERRED — finding:** `StepContext.current`/`dipole`
  are ALREADY `inq::vector3<double>` (vector units), so the TODO's core intent is
  met. Switching to `inqkit::detail::Vec3` would either add an INQ dependency to
  the *pure* `detail/vec3.hpp` (it has `.x/.y/.z` but no `operator[]`, which
  observables_writer + run.cpp rely on) or force a conversion at every per-step
  callback — cosmetic consistency for real coupling risk. Not worth doing unattended.
- **D2 (N-dim plane screen) DEFERRED:** legitimate additive feature, but verifying
  it needs a ~20-min engine-suite rebuild on a coronene-critical header, and that
  GPU/time is better spent on E (the prioritised finale). Spec preserved in
  `docs/plans/new-features-backlog.md`. Do D1+D2 in a supervised session where the
  slow build cycles are watched.

### Group E — complex-run validation (in progress)
Bit-identical re-runs vs golden (run.cpp physics CSVs; fft_*/spectrum CSVs are
analyse.py leftovers, not regenerated by inq-run, so trivially 0).
- **E2-WP** `run_wp_n162_L30_E50_highdens_sigma1` ✅ wall 732s; worst |Δ|=**1e-11**
  (observables 1e-13, momentum_distribution 1e-15, occupations 0). PASS.
- **E2-classical** `run_classical_n162_L30_E50_highdens` ✅ wall 1147s; worst
  |Δ|=**1e-13** (observables 1e-13, electron_track 2e-16, occupations 0). PASS.
  → **E2 highdens pair both PASS.**
- **E1 coronene** `run_cc_bond` ✅ wall 2907s; observables worst |Δ|=**1e-12**.
  PASS. ⚠ LEED `.dat` screens NOT compared (only CSV golden backed up; re-run
  overwrote the .dat) — deterministic from the bit-identical propagation, so
  almost certainly identical but UNVERIFIED. (For future E-runs: back up .dat too.)
- **E3-WP** `run_wp_n162_L50_E20` ✅ wall 8332s; worst |Δ|=**1e-13** (observables
  1e-13, momentum_distribution 1e-18, occupations 0). PASS. (true low-E regime)
- **E3-classical** `run_classical_n162_L50_E20` ⏳ running (GPU0, ~2.5h remaining).

- **E3-classical** `run_classical_n162_L50_E20` ✅ wall 19429s; worst |Δ|=**8e-13**
  (observables 8e-13, electron_track 2e-14, occupations 0). PASS. (true low-E)

**Group E: 5/5 PASS.** Migration confirmed BIT-IDENTICAL (FP noise ≤1e-11) across
vacuum / jellium-WP / jellium-classical / coronene-LEED / high+low density /
true-low-E. (8 production runs total validated incl. the 3 earlier sanity runs.)

---

## FINAL SUMMARY (overnight run complete)

**Done & committed (12 commits on `unit-tests/inq-stack`, no push):**
- Step 0 baseline (6 commits); Group A glue 3/3; Group B B1+B3; Group C C5/C6/C8;
  A1 plasmon-3d, A2 COD, A3 gs-proj, B1 wake-split, B3 KL-split. Suite **91 pass**.
- Group E: **5/5 complex runs bit-identical** to golden.

**Deferred (documented, NOT failures — each a back-compat/risk/value call):**
- B2 energy_balance (untested embedded compute); C1 config/defaults, C3 vti, C4
  relocate apps, C7 dispatcher rename (all back-compat / keep-shim conflicts);
  D1+D2 inqkit C++ features (D1 intent already met by inq::vector3; D2 needs a
  watched engine-build session). All carried to phase-3 with rationale.

**Known gap:** E1 coronene LEED `.dat` screens not compared (only CSV golden
backed up; re-run overwrote the .dat). Observables bit-identical (1e-12) so the
screens — deterministic from the same propagation — are almost certainly
identical but UNVERIFIED.

**Tree never left red.** Every code step gated (pytest+compile+deps-clean) and
committed only when green.
