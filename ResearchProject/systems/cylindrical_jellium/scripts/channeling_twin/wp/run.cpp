// ============================================================================
// cylindrical_jellium / scripts/channeling_twin / wp / run.cpp
//
// QUANTUM HALF of the channeling twin: an electron WAVEPACKET (sigma_WP = 4 Bohr,
// drift k0 = 1.917 = 50 eV) injected as an occupied KS orbital and launched
// on-axis at z = -28 down the hollow bore of the periodic r_s = 3 annular jellium
// tube. Plan: docs/plans/cylindrical-channeling-ks-stopping.md
//
...
//   * 2 sigma_d reaches the R_in = 10 bore wall at t = 23.3 a.u. (step 1165), so
//     the defensible fit window ends well before the run does. f_bore(t) is the
//     measured version of this and the analysis gates on it, not on the formula.
...
int main(){
    ...
    std::cout << std::setprecision(10)
              ...
              << std::sqrt(2.0*SIGMA_WP*SIGMA_WP*((Cfg::R_IN_BOHR/2.0)*(Cfg::R_IN_BOHR/2.0)
    ...
    if(START == 0){
        ...
        // The channeling premise, at t = 0: Rayleigh gives
        // f_bore = 1 - exp(-R_in^2/(2 sigma_d^2)) = 0.9981 for R_in=10, sigma_d=2.83.
    ...
    if(electrons.root()){
        ...
        s << std::setprecision(14)
          << "run = cylindrical_jellium/channeling_twin/wp/" << env_s("CH_OUT","wp") << "\n"
