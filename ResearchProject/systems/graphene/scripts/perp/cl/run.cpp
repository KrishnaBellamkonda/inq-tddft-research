...
#include "/local/data/public/skcb2/tddft/ResearchProject/systems/graphene/shared/configs/graphene_perp_coronene_gs.hpp"
...
int main(){
  ...
  // z_valence=-1 (consistent with its +1/r repulsive local potential) -> proper
  // -1 Ewald charge -> feels the carbon NUCLEI (interaction.hpp:329).
  // SYMBOL = "He" (NOT "H"): the coronene flake already contains H atoms, and
  // INQ keys species by SYMBOL, so a "H" projectile would COLLIDE with the
  // flake's H and silently inherit the default H pseudo (z_val=+1, a proton).
  // A distinct symbol keeps the custom z_valence=-1 electron UPF. (Perpendicular
  // graphene had no H, so "H" worked there.)
  auto sp=ionic::species("He").pseudo_file(
            "/local/data/public/skcb2/tddft/ResearchProject/systems/graphene/shared/pseudopotentials/electron_gaussian_sigma1p47_He.upf")
          .mass(1.0/1822.8885);
  ...
  // Stopping from the member's ACTUAL initial KE (jittered v0), NOT nominal E_eV
  // (the old E_eV-KE_f gave wrong KE_loss for jittered ensemble members).
  const double KE_0=0.5*(vx*vx+vy*vy+vz*vz)*HA_TO_EV;
  { std::ofstream s(outdir+"/run_summary.txt"); s.precision(12);
    s<<"run_completed = true\nrun_type = graphene-classical-cap-grazing\ntag = "<<tag<<"\nE_eV = "<<E_eV
     <<"\nk0 = "<<k0<<"\ncap = "<<(int)capon<<"\neta_Ha = "<<eta<<"\nL_cap = "<<Lcap<<"\nseed = "<<seed
     <<"\nsigma = "<<sigma<<"\nimpact_b_bohr = "<<cx<<"\nproj_r0 = "<<px<<' '<<py<<' '<<pz<<"\nproj_v0 = "<<vx<<' '<<vy<<' '<<vz
     <<"\nproj_rf = "<<pf[0]<<' '<<pf[1]<<' '<<pf[2]<<"\nproj_vf = "<<vf[0]<<' '<<vf[1]<<' '<<vf[2]
     <<"\nKE_initial_eV = "<<KE_0<<"\nKE_final_eV = "<<KE_f<<"\nKE_loss_eV = "<<(KE_0-KE_f)<<"\nn_carbon = "<<n_carbon<<"\nproj_idx = "<<proj
     <<"\ndt = "<<dt<<"\nN_STEPS = "<<N_STEPS<<"\nwrite_every = "<<WRITE_EVERY
     <<"\ncell_bohr = "<<cfg::LX_BOHR<<' '<<cfg::LY_BOHR<<' '<<cfg::LZ_BOHR
     <<"\npropagator = etrs (Ehrenfest)\nwall_s = "<<wall<<"\nPROVISIONAL = inq-study Task#7\n"; }
  std::printf("  [%s] b=%.2f KE_init=%.2f KE_final=%.2f KE_loss=%.2f eV wall=%.1fs -> %s\n",tag.c_str(),cx,KE_0,KE_f,KE_0-KE_f,wall,outdir.c_str());
  return 0;
}
