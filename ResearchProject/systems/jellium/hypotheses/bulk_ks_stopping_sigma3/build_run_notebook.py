...
CFG = dict(
    LX=46.0, LY=46.0, LZ=80.0, N_E=218, DX=0.4,
    ...
    T_IFW=19.4845, T_TRANSVERSE=31.2570, COMMON_FIT_T1=9.37,
    R_S=5.702, HW_P_EV=3.462, V_F=0.3366, PLASMA_PERIOD=49.39,
)
...
def build(half: str) -> nbf.NotebookNode:
    is_wp = half == "wp"
    run_dir = SYSTEM / "scripts/bulk_ks_stopping_sigma3" / half
...
def main() -> int:
    ...
    half = sys.argv[1]
    nb = build(half)
    out = HERE / f"bulk_ks_stopping_sigma3_{half}.ipynb"
