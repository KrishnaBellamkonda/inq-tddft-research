#!/usr/bin/env python3
"""Build (and execute) the run notebook for one half of the bulk-jellium
KS-stopping twin pair.
...
# shared/configs/bulk_ks_stopping_L46x46x80.hpp. Kept as literals so the
# notebook is readable standalone; the run_summary.txt printed in the setup cell
# is the authority if they ever disagree.
CFG = dict(
    LX=40.0, LY=40.0, LZ=80.0, N_E=482, DX=0.5,
    SIGMA_WP=3.0, EKIN_EV=100.0, K0=2.7111, Z0=-28.0,
    DT=0.04, N_STEPS=600,
    # boundary rule z0 = -Lz/2 + 4 sigma, so z0 and N_STEPS differ per sigma;
    # cross-sigma comparison must use the COMMON window [4, 9.37], not raw path.
    FIT_T0=4.0, FIT_T1=19.4845,
    T_IFW=19.4845, T_TRANSVERSE=26.8142, COMMON_FIT_T1=9.37,
    R_S=3.987, HW_P_EV=5.919, V_F=0.4813, PLASMA_PERIOD=28.88,
)
...
def build(half: str) -> nbf.NotebookNode:
    is_wp = half == "wp"
    run_dir = SYSTEM / "scripts/bulk_ks_stopping_rs4_sigma3" / half
...
`static_assert` in `shared/configs/bulk_ks_stopping_L46x46x80.hpp`. Ground state
...
def main() -> int:
    ...
    half = sys.argv[1]
    nb = build(half)
    out = HERE / f"bulk_ks_stopping_rs4_sigma3_{half}.ipynb"
