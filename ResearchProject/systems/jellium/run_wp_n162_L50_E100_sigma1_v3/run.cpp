...
using Cfg = jellium::config::Electron_Proj_E100_L50_sigma1_WP_dx0p40;
...
int main() {
    ...
    const std::string RUN_NAME = "run_wp_n162_L50_E100_sigma1_v3";
    ...
    std::filesystem::create_directories("results/raw/vti/orbitals");
    ...
    //     per snapshot)
    //
    // v3: WP orbital complex VTI at first and last frames only.
    inqkit::io::ComplexField3DWriter wp_orb_wr(
    ...
    // Save WP orbital at t=0
    {
        auto wp_psi = inqkit::fields::orbital::wavefunction(electrons, wp_idx);
        wp_orb_wr.write(wp_psi, "wp_orbital_t0");
        std::cout << "  Saved WP orbital at t=0\n";
    }
    ...
    // v3: Save WP orbital at t_end
    {
        auto wp_psi = inqkit::fields::orbital::wavefunction(electrons, wp_idx);
        wp_orb_wr.write(wp_psi, "wp_orbital_tend");
        std::cout << "  Saved WP orbital at t_end\n";
    }
