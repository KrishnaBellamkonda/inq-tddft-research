// ============================================================================
// run_classical_e1000_L40x40x150/run.cpp — classical-electron projectile
// (custom UPF + mass override = m_e) through the elongated 40x40x150 Bohr
// orthorhombic jellium bath at N=162, dx=0.30. Companion of
// run_wp_e1000_L40x40x150/ — same KE, same v_z, same launch position.
//
// Cfg: jellium::config::Electron_Proj_E1000_L40x40x150_Classical.
// Velocity match with WP is automatic: KE = 1000 eV (= 36.75 Ha), m = 1
// atomic unit (= m_e via mass override), so v_z = sqrt(2 * 36.75) = 8.5732
// bohr/atu, identical to WP_K0.
//
// Pseudopotential: shared/pseudopotentials/electron-ONCV-1.2.upf
//   (byte-identical to anti-proton-ONCV-1.2.upf, header comment only).
...
using Cfg = jellium::config::Electron_Proj_E1000_L40x40x150_Classical;
...
int main() {
    ...
    const std::string RUN_NAME = "run_classical_e1000_L40x40x150";
    ...
    // electron_track.csv: projectile pos / vel / force every step.
    ...
    // Full-matrix overlap snapshot at t=0 (bath self-overlap, should be ≈ I).
