// ============================================================================
// run_free_wp_L30_E100_sigma1/run.cpp — free-WP reference for high-density jellium.
//
// FREE-SPACE wave-packet propagation in a 30^3 Bohr PERIODIC box,
// non-interacting theory (no Hartree, no XC). Same WP injection geometry
// as the high-density jellium σ=1 runs so density VTIs can be
// subtracted frame-by-frame:
//
...
//   N_STEPS = 475                  (boundary_rule.n_steps_for at k0)
...
namespace cfg {
    ...
    // Self-spread capped at 475 steps (vs boundary_rule.n_steps_for ≈ 830);
    // see plan / Cfg header. At σ=1 the WP density tail reaches the box edge
    // at t ≈ 9.5 a.u., well before the centroid-criterion stop.
    constexpr int N_STEPS     = 268;
    constexpr int WRITE_EVERY = jellium::config::boundary::write_every_for(N_STEPS);
}
...
int main() {
    ...
    const std::string RUN_NAME = "run_free_wp_L30_E100_sigma1";

    std::cout << "\n=== " << RUN_NAME << " (Run-2: free-space WP @ 100 eV) ===\n"
              << "  cell    = " << cfg::L_BOHR << "^3 Bohr (periodic, NON-interacting)\n"
    ...
    // Density VTI series (needed for the density GIF in the
    // free-vs-jellium comparison email at σ=1). density::total is the
    // cheap bulk-copy extraction — under non-interacting theory it is
    // |psi_wp|² + |psi_ghost|² but the ghost is uniform-ish so the WP
    // motion dominates the visual.
    std::filesystem::create_directories("results/raw/vti/density_rt_total");
    std::filesystem::create_directories("results/raw/vti/density_rt_delta");
    std::filesystem::create_directories("results/raw/vti/density_rt_delta_coarse");
    ...
    inqkit::io::RealField3DWriter total_wr(
        "results/raw/vti/density_rt_total", vti_layout, {.overwrite=true});
    inqkit::observables::DensityDelta density_delta(
        ...
        {.emit_raw_vti = true, .emit_coarse_vti = true,
         .compute_l2 = false, .coarse_bin_bohr = 3.0});
