...
namespace cfg {
    ...
    // Timestep — matches the draft3 validation run run_free_wp_L50_E100
    constexpr double DT_AU = 0.020;

    // WP — replicates run_free_wp_L50_E100 (the draft3 spreading-validation run)
    constexpr double WP_EKIN_EV    = 100.0;
    constexpr double WP_SIGMA_BOHR = 5.0;        // sigma=5 -> density sigma_r(0)=3.536, clean spreading
    // k0 = sqrt(2 * E_eV / 27.21138625)
    constexpr double HA_TO_EV      = 27.21138625;
    constexpr double WP_K0         = 2.71106334010243; // sqrt(2*100/27.21138625)
    ...
    constexpr double WP_CZ_BOHR =
        jellium::config::boundary::launch_z(WP_SIGMA_BOHR, L_BOHR); // -5 for sigma=5

    // Duration — matches run_free_wp_L50_E100 (t_total = 9.24 a.u.)
    constexpr int N_STEPS      = 462;
    constexpr int WRITE_EVERY  = 2;   // ~231 density frames (matches original)
    constexpr int WF_WRITE_EVERY = 10;
}
...
int main() {
    ...
    const std::string RUN_NAME = "run_free_wp_L50_E100_dens";

    std::cout << "\n=== " << RUN_NAME << " (free-space WP @ 100 eV, sigma=5, density VTI) ===\n"
