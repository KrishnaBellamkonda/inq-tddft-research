// ============================================================================
// run_wp_n162_L50_E100_sigma1/run.cpp — Gaussian wave-packet projectile in a cubic
...
//
// Cfg: jellium::config::Electron_Proj_E100_L50_sigma1_WP_dx0p40.
...
// Bohr, KE=100 eV. N_STEPS=475 (self-spread-capped), WRITE_EVERY=2 (~231 frames).
//
...
#include "../shared/configs/electron_proj_E100_L50_cubic_sigma1_v2.hpp"
...
using Cfg = jellium::config::Electron_Proj_E100_L50_sigma1_v2_WP;
...
int main() {
    ...
    const std::string RUN_NAME = "run_wp_n162_L50_E100_sigma1_v2";
    ...
    // v2: WP density and wavefunction VTI writers ENABLED.
    // density::orbital is expensive (per-element GPU→host) but we write at
    // WF_WRITE_EVERY (less frequent than density_total at WRITE_EVERY).
    ...
    //     per snapshot)
    //
    // v2: WP density + wavefunction ENABLED at WF_WRITE_EVERY cadence.
    // density::total (bulk-copy) at WRITE_EVERY (cheap).
    // density::orbital + wavefunction at WF_WRITE_EVERY (expensive but
    // needed for exact WP subtraction, momentum distribution, and SE analysis).
    ...
    rt_obs.add([&](inqkit::StepContext const &ctx) {
        ...
        // WP density + wavefunction at WF_WRITE_EVERY cadence.
