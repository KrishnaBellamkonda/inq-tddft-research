// ============================================================================
// save_gs/gs_L60_cubic_N138_dx0p55/run.cpp
//
// Jellium GS at cell = 60^3 Bohr cubic-periodic, N = 138 (closed-shell magic
// |G|^2 <= 6). Density 6.39e-4 e/bohr^3, r_s ~= 7.21 bohr (near the legacy
// Base r_s of 7.38 but on a fully-filled shell so the GS density is uniform
// by the closed-shell symmetry argument).
//
// Spacing 0.55 Bohr (vs Base 0.50): coarsened to reduce per-step propagator
// cost while still satisfying the Nyquist condition for a 100 eV WP with
// sigma = 1.0015 Bohr (k_max = k_0 + 3*sigma_k = 5.71 Bohr^-1; required
// dx <= pi / k_max = 0.5505 Bohr).
//
// Cfg = jellium::config::Base_N138 (defined in shared/configs/base_n138.hpp).
// ============================================================================
#include <inq/inq.hpp>
...
using Cfg = jellium::config::Base_N138;
...
int main() {
    ...
    std::cout << "\n=== save_gs/gs_L60_cubic_N138_dx0p55 ===\n"
    ...
    if (electrons.root()) {
        ...
        summary << "run = save_gs/gs_L60_cubic_N138_dx0p55\n"
                << "system = jellium_N138_closed_shell_lowE_WP\n"
