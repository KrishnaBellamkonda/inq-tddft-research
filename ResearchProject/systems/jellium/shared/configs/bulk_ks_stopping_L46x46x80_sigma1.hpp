// ============================================================================
// shared/configs/bulk_ks_stopping_L46x46x80_sigma1.hpp
//
...
// = 15.71, T_TRANSVERSE (periodic images) = 10.80; FIT_T1 = min = 10.80.
...
//
// GRID. dx = 0.4 unchanged. k_max = k0 + 3 sigma_p = 4.832 against
// k_Nyquist = 7.854 (1.63x margin). VERIFIED numerically on this exact
...
struct Common_Bulk_KS_L46x46x80_sigma1 : Common_Bulk_KS_Stopping_L46x46x80 {
    ...
    static constexpr double T_TRANSVERSE_AU =
        boundary::transverse_clean_t(WP_SIGMA_BOHR, LX_BOHR);    // 10.80
...
struct Bulk_KS_L46x46x80_sigma1_WP : Common_Bulk_KS_L46x46x80_sigma1 {
    static constexpr bool WP_ENABLED = true;
    static constexpr int  N_IONS     = 0;
};

struct Bulk_KS_L46x46x80_sigma1_Classical : Common_Bulk_KS_L46x46x80_sigma1 {
    static constexpr bool WP_ENABLED = false;
    static constexpr int  N_IONS     = 1;
};

// ---- Compile-time guards --------------------------------------------------
static_assert(Common_Bulk_KS_L46x46x80_sigma1::N_STEPS == 692,
              "sigma1 L46x46x80: N_STEPS changed - re-derive the header numbers");
static_assert(Common_Bulk_KS_L46x46x80_sigma1::WP_CZ_BOHR == -36.0,
              "sigma1 L46x46x80: launch z should be -L_z/2 + 4 sigma");
...
static_assert(Common_Bulk_KS_L46x46x80_sigma1::FIT_T1_AU
                  - Common_Bulk_KS_L46x46x80_sigma1::FIT_T0_AU > 5.0,
              "sigma1 L46x46x80: fit window shorter than 5 a.u. - too short for a slope");
// The cross-sigma common window must actually be inside this run's valid window.
static_assert(Common_Bulk_KS_L46x46x80_sigma1::COMMON_FIT_T1_AU
                  <= Common_Bulk_KS_L46x46x80_sigma1::FIT_T1_AU + 1e-9,
              "sigma1 L46x46x80: the common cross-sigma window is NOT clean for this "
              "run - it extends past this run's own interference limit");
// Nyquist guard at the new (larger) momentum width.
static_assert(Common_Bulk_KS_L46x46x80_sigma1::WP_K0
                  + 3.0 / (2.0 * Common_Bulk_KS_L46x46x80_sigma1::WP_SIGMA_BOHR)
                  < 3.14159265358979
                        / Common_Bulk_KS_L46x46x80_sigma1::SPACING_BOHR,
              "sigma1 L46x46x80: grid too coarse for the WP bandwidth");
