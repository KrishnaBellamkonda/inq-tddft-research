// ============================================================================
// electron_proj_E20_L50_cubic_sigma1_v2.hpp — v2 rerun with:
...
//
// Self-spread bound (σ=1, L=50, v=1.212):
//   -21 + 1.212·t + 2.121·√(1+t²) = 25  →  t* ≈ 11.4 a.u.
//   N_STEPS = 1140 (was 475 at dt=0.02, but self-spread cap at v=1.212
//   gives a longer window than at v=2.711).
//
// Nyquist: k₀=1.212, σ_k=0.707. k_max=3.33. k_Nyq=7.85. Ratio 0.42 — OK.
...
struct Common_E20_L50_sigma1_v2 : Base_N162_L50_E1p5 {
    ...
    // Self-spread-capped: t* ≈ 11.4 a.u. at dt=0.01 → 1140 steps
    static constexpr int N_STEPS     = 1140;
    static constexpr int WRITE_EVERY = 4;              // ~285 density frames

    // WP wavefunction saving cadence (heavier I/O — less frequent)
    static constexpr int WF_WRITE_EVERY = 10;          // ~114 wavefunction frames
...
struct Electron_Proj_E20_L50_sigma1_v2_WP : Common_E20_L50_sigma1_v2 {
    static constexpr bool WP_ENABLED = true;
    static constexpr int  N_IONS     = 0;
};

struct Electron_Proj_E20_L50_sigma1_v2_Classical : Common_E20_L50_sigma1_v2 {
    static constexpr bool WP_ENABLED = false;
    static constexpr int  N_IONS     = 1;
    // Boundary-rule N_STEPS: traversal=45 / (v=1.212 * dt=0.01) → 3713
    static constexpr int N_STEPS     = 3713;
    static constexpr int WRITE_EVERY = 12;             // ~309 density frames
};
