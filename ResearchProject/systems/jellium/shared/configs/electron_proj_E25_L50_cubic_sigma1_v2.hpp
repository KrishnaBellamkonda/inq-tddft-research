// ============================================================================
// electron_proj_E25_L50_cubic_sigma1_v2.hpp — v2 rerun with:
...
//
// Self-spread bound (σ=1, L=50, v=1.356):
//   -21 + 1.356·t + 2.121·√(1+t²) = 25  →  t* ≈ 11.0 a.u.
//   N_STEPS = 1100 at dt=0.01.
//
// Nyquist: k₀=1.356, σ_k=0.707. k_max=3.48. k_Nyq=7.85. Ratio 0.44 — OK.
...
struct Common_E25_L50_sigma1_v2 : Base_N162_L50_E1p5 {
    ...
    // Self-spread-capped: t* ≈ 11.0 a.u. at dt=0.01 → 1100 steps
    static constexpr int N_STEPS     = 1100;
    static constexpr int WRITE_EVERY = 4;              // ~275 density frames

    // WP wavefunction saving cadence (heavier I/O — less frequent)
    static constexpr int WF_WRITE_EVERY = 10;          // ~110 wavefunction frames
...
struct Electron_Proj_E25_L50_sigma1_v2_WP : Common_E25_L50_sigma1_v2 {
    static constexpr bool WP_ENABLED = true;
    static constexpr int  N_IONS     = 0;
};

struct Electron_Proj_E25_L50_sigma1_v2_Classical : Common_E25_L50_sigma1_v2 {
    static constexpr bool WP_ENABLED = false;
    static constexpr int  N_IONS     = 1;
    // Boundary-rule N_STEPS: traversal=45 / (v=1.356 * dt=0.01) → 3319
    static constexpr int N_STEPS     = 3319;
    static constexpr int WRITE_EVERY = 11;             // ~302 density frames
};
