// ============================================================================
// shared/configs/bulk_ks_stopping_L40x40x80_rs4_sigma3.hpp
//
...
// = 19.48, T_TRANSVERSE (periodic images) = 26.81; FIT_T1 = min = 19.48.
...
//
// GRID. dx = 0.5 unchanged. k_max = k0 + 3 sigma_p = 3.418 against
// k_Nyquist = 6.283 (1.84x margin). VERIFIED numerically on this exact
...
struct Common_Bulk_KS_L40x40x80_rs4_sigma3 : Common_Bulk_KS_Stopping_L40x40x80_rs4 {
    ...
    static constexpr double T_TRANSVERSE_AU =
        boundary::transverse_clean_t(WP_SIGMA_BOHR, LX_BOHR);    // 26.81
...
struct Bulk_KS_L40x40x80_rs4_sigma3_WP : Common_Bulk_KS_L40x40x80_rs4_sigma3 {
    static constexpr bool WP_ENABLED = true;
    static constexpr int  N_IONS     = 0;
};

struct Bulk_KS_L40x40x80_rs4_sigma3_Classical : Common_Bulk_KS_L40x40x80_rs4_sigma3 {
    static constexpr bool WP_ENABLED = false;
    static constexpr int  N_IONS     = 1;
};

// ---- Compile-time guards --------------------------------------------------
static_assert(Common_Bulk_KS_L40x40x80_rs4_sigma3::N_STEPS == 600,
              "sigma3 L40x40x80_rs4: N_STEPS changed - re-derive the header numbers");
static_assert(Common_Bulk_KS_L40x40x80_rs4_sigma3::WP_CZ_BOHR == -28.0,
              "sigma3 L40x40x80_rs4: launch z should be -L_z/2 + 4 sigma");
...
static_assert(Common_Bulk_KS_L40x40x80_rs4_sigma3::FIT_T1_AU
                  - Common_Bulk_KS_L40x40x80_rs4_sigma3::FIT_T0_AU > 5.0,
              "sigma3 L40x40x80_rs4: fit window shorter than 5 a.u. - too short for a slope");
// The cross-sigma common window must actually be inside this run's valid window.
static_assert(Common_Bulk_KS_L40x40x80_rs4_sigma3::COMMON_FIT_T1_AU
                  <= Common_Bulk_KS_L40x40x80_rs4_sigma3::FIT_T1_AU + 1e-9,
              "sigma3 L40x40x80_rs4: the common cross-sigma window is NOT clean for this "
              "run - it extends past this run's own interference limit");
// Nyquist guard at the new (larger) momentum width.
static_assert(Common_Bulk_KS_L40x40x80_rs4_sigma3::WP_K0
                  + 3.0 / (2.0 * Common_Bulk_KS_L40x40x80_rs4_sigma3::WP_SIGMA_BOHR)
                  < 3.14159265358979
                        / Common_Bulk_KS_L40x40x80_rs4_sigma3::SPACING_BOHR,
              "sigma3 L40x40x80_rs4: grid too coarse for the WP bandwidth");
