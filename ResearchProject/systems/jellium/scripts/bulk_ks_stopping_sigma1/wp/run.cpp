// ============================================================================
// scripts/bulk_ks_stopping/wp/run.cpp
//
...
// is injected as an extra KS orbital into a fully periodic 46 x 46 x 80 Bohr
// jellium bath (N = 218, r_s = 5.702) and propagated 646 steps at dt = 0.04
...
#include "../../../shared/configs/bulk_ks_stopping_L46x46x80_sigma1.hpp"
...
using Cfg = jellium::config::Bulk_KS_L46x46x80_sigma1_WP;
...
int main() {
    ...
    const std::string RUN_NAME = "bulk_ks_stopping_sigma1_wp";
    ...
    if (!std::filesystem::exists(GS_DIR)) {
        std::cerr << "FATAL: GS checkpoint missing at " << GS_DIR
                  << " — run save_gs/gs_L46x46x80_orth_N218_dx0p40 first.\n";
        return 2;
    }
    ...
    if (START == 0) {
        ...
        // sigma_z: PAULI-BROADENED, so the free-Gaussian value is a LOWER bound,
        // not the expectation. Diagnosed 2026-07-31 after this gate aborted job
        // 32482671 at sigma_WP = 1 (measured 0.7680 vs free 0.7071, +8.6%).
        //
        // It is NOT a grid artefact: a pure sampled Gaussian reproduces
        // sigma_d = sigma/sqrt2 to 0.00% on BOTH the dx=0.4 and dx=0.5 grids
        // (checked numerically). The broadening is physical. At sigma_WP = 1 the
        // packet's momentum width is sigma_p = 1/(sigma*sqrt2) = 0.707, which
        // overlaps the occupied Fermi sphere (k_F = 0.481 here, 0.337 at
        // r_s = 5.702), so Gram-Schmidt removes real low-k weight — max_overlap
        // is 1.6e-3, up 400x from the 3.1e-6 of the sigma = 2 runs. Stripping
        // low-k components leaves long real-space tails, and <z^2> weights tails
        // heavily. Confirmed by the ordering: the DENSER bath has the larger
        // Fermi sphere (k_F/sigma_p 0.681 vs 0.476) and shows the larger
        // broadening (+8.6% vs +3.2%).
        //
        // The SHARP gates on the packet are the momentum ones above (sigma_pz^2,
        // T1, T2, T1-T2), which pass to 0.1%, plus the exact circular centroid.
        // This one is kept as a coarse sanity bound: it must still catch a packet
        // injected at the wrong sigma (2x wrong => 1.414 vs 0.707, far outside),
        // while accepting orthogonalisation broadening. Orthogonalisation can
        // only ever BROADEN, so the asymmetric bound below is the honest form.
        {
            const double sz    = std::sqrt(r0.sz2);
            const double sfree = Cfg::WP_SIGMA_BOHR / std::sqrt(2.0);
            const bool ok = (sz >= sfree - 0.02) && (sz <= 1.25 * sfree);
            std::cout << (ok ? "  [PASS] " : "  [FAIL] ")
                      << "sigma_z (density std, Pauli-broadened): got " << sz
                      << ", free-Gaussian " << sfree << ", allowed ["
                      << sfree - 0.02 << ", " << 1.25 * sfree << "]"
                      << "  (max_overlap = " << report.max_overlap << ")\n";
            if (!ok) ++fails;
        }
        if (fails > 0) {
