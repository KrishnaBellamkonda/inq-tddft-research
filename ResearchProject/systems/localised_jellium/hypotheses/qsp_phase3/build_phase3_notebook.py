#!/usr/bin/env python3
"""Assemble qsp_phase3_study.ipynb — the BIG-BOX production study notebook
...
Thin narrative assembler: every figure + GIF is pre-computed by analyse_phase3.py
...
Run:
  ...
  /local/data/public/skcb2/tddft/venv/bin/python3 build_phase3_notebook.py
"""
import json, os, sys
...
# ----------------------------------------------------------------- §0 title
cells.append(md(rf"""# Phase 3 · P3.1 — big-box production: WP vs classical stopping (energy method)
### localised jellium slab · 50×50×**90** · r$_s$≈5.67 · σ$_{{\rm WP}}$=0.5 · 100 eV · two-sided CAP η=−0.7 · τ=100 a.u.
...
| k₀ / v₀ | WP wavevector = projectile speed | 2.711 (½v₀²≈100 eV) |
...
| ⟨T$_{\rm WP}$⟩ | WP kinetic = ½k₀² + 3/(4σ²) | 6.675 Ha (181.6 eV) |
...
| **WP** | Gaussian σ=0.5, k₀=2.711, last extra state, orthogonalised against occupied |
| **classical** | Ehrenfest ghost ion, Gaussian-e UPF σ=0.354, v=(0,0,2.711) |
...
| file | role |
|---|---|
| `scripts/qsp_phase3/wp/run.cpp` | WP run (ETRS, two-sided CAP, full observable suite) |
| `scripts/qsp_phase3/classical/run.cpp` | classical Ehrenfest ghost-ion run |
| `scripts/qsp_phase3/run_production.sh` | concurrent launch (GPU0/GPU1), env-driven dt/steps |
| `shared/configs/slab_n82_L50x50x90.hpp` | geometry / density / WP config struct |
| `shared_gs/slab_n82_L50x50x90/` | 90-box ground state |
| `hypotheses/qsp_phase3/analyse_phase3.py` | computes every figure + `results.json` |
| `hypotheses/qsp_phase3/p3wp_run_notebook.ipynb` / `p3cl_run_notebook.ipynb` | per-run deep-dives |
"""))
...
- **Classical face-window S = {g('classical_S_facewindow_eVbohr','{:.3f}')} eV/Bohr** (re-extract via the
  stopping skill); **point-Lindhard = {g('S_lindhard_point_100eV_eVbohr','{:.3f}')} eV/Bohr**.
- **CAP (new two-sided, 90-box):** reflection {g('reflect_cum','{:.3f}')}, transmission
  {g('transmit_cum','{:.3f}')}; init-absorption fixed by the equidistant launch.
...
*If the convergence gate did not pass, S$_{{\rm WP}}$ is a bound — extend τ.*
"""))

out = os.path.join(HERE, "qsp_phase3_study.ipynb")
