#!/usr/bin/env python3
"""Phase-4 analysis — big-box (50×50×90) WP + classical, two-sided CAP, τ=100 a.u., v₀=2.0 (54 eV).
...
Run (production = default; smoke = P4_TAG=p4v for testing while production runs):
  P4_TAG=p4 PYTHONPATH=/local/data/public/skcb2/tddft/inq-stack/python \
  /local/data/public/skcb2/tddft/venv/bin/python3 analyse_phase4.py
"""
...
_spk_path = "/local/data/public/skcb2/tddft/.claude/skills/stopping-power-extraction/stopping_power.py"
_spec = _ilu.spec_from_file_location("spk", _spk_path)
spk = _ilu.module_from_spec(_spec); _spec.loader.exec_module(spk)   # stopping-power skill
style.apply_theme()

HA = 27.211386
ROOT = "/local/data/public/skcb2/tddft/ResearchProject/systems/localised_jellium"
TAG = os.environ.get("P4_TAG", "p4")             # "p3" = production, "p4v" = smoke
WP_RUN = f"{ROOT}/scripts/qsp_phase4/wp/results/{TAG}_wp"
CL_RUN = f"{ROOT}/scripts/qsp_phase4/classical/results/{TAG}_classical"
WP = f"{WP_RUN}/raw"
CL = f"{CL_RUN}/raw"
...
SIGMA_WP = 0.5
Z0, V0, RS = -23.75, 2.0, 5.666          # equidistant launch, projectile speed, density
...
try:
    ...
    kF = L.kF_from_rs(RS)
    R["S_lindhard_point_54eV_eVbohr"] = HA * L.stopping_power_point(V0, kF)
    log(f"point-Lindhard S(54eV, r_s5.67) = {R['S_lindhard_point_54eV_eVbohr']:.3f} eV/Bohr")
except Exception as e:
    log(f"[lindhard skip] {e}"); R["S_lindhard_point_54eV_eVbohr"] = None
...
# ===== 3c. CLASSICAL ENERGY METHOD (WP-like, via stopping-power skill) =====
@guard("classical_energy_method")
def classical_energy_method():
    o = pd.read_csv(f"{CL}/observables/observables.csv")
    t, E = o["time_au"].values, o["energy_total"].values
    summ = open(glob.glob(f"{CL}/../run_summary.txt")[0]).read()
    pk = re.search(r"park_triggered = (\w+)", summ)
    park_ok = bool(pk and pk.group(1) == "true")
    pst = re.search(r"park_t_au = ([-\d.]+)", summ)
    park_t = float(pst.group(1)) if pst else -1.0
    R["classical_park_triggered"] = park_ok; R["classical_park_t_au"] = park_t

    # WP-like: deposited = E_total(t_f) − E_GS (clean ONLY post-park: projectile
    # removed -> reversible interaction gone). Anchor t_0 at E_GS so the skill's
    # tail-gate checks the post-park plateau, and dE = the retained deposit.
    spk_t = np.concatenate([[t[0] - DT], t])
    spk_E = np.concatenate([[E_GS], E]) * HA                 # eV
    sp = spk.slab_stopping_power(spk_t, spk_E, L_SLAB, converge_frac=0.15, converge_tol=0.05)
    deposited = float((E[-1] - E_GS) * HA)
    R["classical_deposited_eV"] = deposited
    R["classical_S_energy_eVbohr"] = float(sp["S"])
    R["classical_S_energy_converged"] = bool(sp["converged"])
    R["classical_S_energy_status"] = sp["status"]
    try:
        nC = pd.read_csv(f"{CL}/observables/electron_number.csv")
        cg = spk.conservation_guard(nC["N_total"].values, tol=0.05)
        R["classical_Nconserved"] = cg["ok"]; R["classical_drained_frac"] = cg["drained_frac"]
    except Exception:
        pass

    fig, ax = plt.subplots(figsize=(7.4, 4.6))
    ax.plot(t, (E - E_GS) * HA, "C3-", lw=1.7, label="E_total(t) − E_GS")
    ax.axhline(deposited, ls=":", color="0.5",
               label=f"deposited = {deposited:.1f} eV ⇒ S = {sp['S']:.2f} eV/Bohr")
    if park_ok and park_t > 0:
        ax.axvline(park_t, ls="--", color="C2", lw=1.2, label=f"park+remove (t={park_t:.1f})")
    ax.set_xlabel("time (a.u.)"); ax.set_ylabel("E_total − E_GS  (eV)")
    ax.set_title(f"Classical energy method (WP-like): S={sp['S']:.2f} eV/Bohr "
                 f"[{sp['status']}; park={'yes' if park_ok else 'NO'}]", fontsize=9)
    ax.legend(fontsize=7, frameon=False); ax.grid(alpha=.25)
    fig.tight_layout(); fig.savefig(f"{FIGS}/classical_energy_method.png", dpi=180); plt.close(fig)

    warn = "" if park_ok else "  [WARNING park NOT fired -> projectile present, S contaminated]"
    log(f"classical energy-method: deposited={deposited:.1f} eV ⇒ S={sp['S']:.2f} eV/Bohr "
        f"[{sp['status']}; park={'yes' if park_ok else 'NO'} t={park_t:.1f}]{warn}")

_fns = [convergence, energy_method, classical, classical_transport, classical_energy_method,
        centroid, reflection, momentum, energetics, excitation, efield,
