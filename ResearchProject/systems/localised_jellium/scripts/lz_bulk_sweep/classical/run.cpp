// ============================================================================
// localised_jellium / scripts/lz_bulk_sweep / classical / run.cpp
//
// CLASSICAL half of the slab->bulk L_slab sweep.
// Plan: docs/plans/jellium-slab-extend-Lz.md
//
// Clone of scripts/sigma56_sv/classical/run.cpp (direct erf/r representation,
// Ehrenfest, CAP on both halves, pairwise ledger, interior checkpoints). That
// binary's geometry was ALREADY fully env-driven; the only changes here are
// (1) defaults come from the LZB_CFG box preset (lzb_boxes.hpp) instead of the
//     105-box Cfg, so a run cannot silently mix one box's L_z with another's
//     launch point;
// (2) EXTRA_STATES becomes a knob defaulting to the preset's value — the
//     sigma56 binary pinned it at 24, which is wrong for N = 60/140 boxes;
// (3) run_summary provenance strings.
// The force, ledger, checkpoint/resume and CAP logic are unchanged — see the
// sigma56 header for the full rationale (direct potential vs Poisson, the twin
// contract, why the classical half carries a CAP).
//
// Env: LZB_CFG(REQUIRED) LJ_LX/LY(35) LJ_LZ(preset) LJ_HALF(preset)
//      LJ_N(preset) LJ_EXTRA_STATES(preset) LJ_EDGE_W(1.0) LJ_PERIODICITY(2)
//      LJ_SPACING(0.40) LJ_SIGMA(family default) LJ_LAUNCH_Z(preset) LJ_K0
//      LJ_MASS(1.0) LJ_N_STEPS LJ_DT(0.04) LJ_SAVE_EVERY LJ_CKPT_EVERY(0=auto)
//      LJ_CAP_ETA(-1.0) LJ_CAP_L(12.5) LJ_GS_DIR(REQUIRED) LJ_OUT(REQUIRED)
//      LJ_RESUME(0)
// ============================================================================
#include <inq/inq.hpp>
...
int main(){
    ...
    // ---- parameters (defaults pinned to the LZB_CFG preset) -----------------
    const double LX=env_d("LJ_LX",Sh::LX_BOHR), LY=env_d("LJ_LY",Sh::LY_BOHR);
    const double LZ=env_d("LJ_LZ",B.LZ_BOHR), HALF=env_d("LJ_HALF",B.SLAB_HALF);
    const int    N=env_i("LJ_N",B.N_ELECTRONS);
    const int    EXTRA=env_i("LJ_EXTRA_STATES",B.EXTRA_STATES);
    const double EDGE_W=env_d("LJ_EDGE_W",Sh::EDGE_WIDTH_BOHR);
    ...
    const double K0=env_d("LJ_K0",3.0), MASS=env_d("LJ_MASS",Sh::PROJ_MASS);
    const int    N_STEPS=env_i("LJ_N_STEPS",2543);
    const double DT=env_d("LJ_DT",0.04);
    const int    SAVE_EVERY=env_i("LJ_SAVE_EVERY",25);
    const double CAP_ETA=env_d("LJ_CAP_ETA",Sh::CAP_ETA_HA);   // 0 => CAP-free control
    const double CAP_L  =env_d("LJ_CAP_L",  Sh::CAP_L_BOHR);
    const std::string GS_DIR=env_s("LJ_GS_DIR","");
    const std::string OUT="results/"+env_s("LJ_OUT","cl_"+B.name);
    ...
    // CAP geometry — FRACTIONAL, derived from the ACTUAL L_z in use.
    ...
    if(RESUME){
        ...
        // The projectile is NOT part of `electrons`; restore R and V by hand.
    ...
    std::cout<<std::setprecision(12)
             <<"\n=== lz_bulk_sweep CLASSICAL ["<<B.name<<"] (direct erf/r, Ehrenfest)  OUT="<<OUT<<" ===\n"
             ...
             <<"  mass="<<MASS<<"  charge="<<Sh::PROJ_CHARGE<<"\n"
    ...
    auto electrons=systems::electrons(ions,options::electrons{}.spacing(SPACING*1.0_b)
        .extra_electrons(N).extra_states(EXTRA).temperature(Sh::TEMPERATURE_EV*1.0_eV),
        input::kpoints::gamma());
    ...
    inqkit::dynamics::Projectile proj(MASS, Sh::PROJ_CHARGE,
    ...
    // eta = 0 makes the absorbing bands an exact identity, so the CAP-free
    ...
    // ---- density frames (canonical raw/vti path + legacy symlink) -----------
    if(SAVE_EVERY>0){
    ...
    rt.add([&](inqkit::StepContext const& ctx){
        ...
        // Ehrenfest: DIRECT erf/r HF force from (electrons - background).
        ...
        // DIRECT-potential pairwise ledger: V_proj built ONCE per step.
        ...
        if(electrons.root()){
            pj<<ctx.step<<","<<ctx.time_au<<","<<Rn.z<<","<<proj.V().z<<","
              <<proj.ke()<<","<<Uprojbg<<","<<Fz<<"\n";
            // norm_slab is the CAP witness on this half.
    ...
    if(electrons.root()){
        ...
        s<<std::setprecision(12)
         <<"run = localised_jellium/lz_bulk_sweep/classical/"<<env_s("LJ_OUT","cl_"+B.name)<<"\n"
         <<"plan = docs/plans/jellium-slab-extend-Lz.md\n"
         <<"engine = inq-study\nxc = LDA\n"
         ...
         <<"  N = "<<N<<"  spacing = "<<SPACING<<"\n"
         <<"slab_half = "<<HALF<<"  slab_thickness = "<<2*HALF
         <<"  edge_w = "<<EDGE_W<<"  n0 = "<<N0<<"  r_s = "<<rs<<"\n"
         <<"extra_states = "<<EXTRA<<"\n"
         <<"launch_z = "<<LAUNCH_Z<<"  standoff_bohr = "<<(-HALF-LAUNCH_Z)
         ...
         <<"analysis_note = deposit S = [E_total(t_f) - E_GS - E_PS(t_f)] / "<<2*HALF
         <<" Bohr (E_PS monopole tail subtracted); per-box E_GS from the gs run\n"
         <<"gs_dir = "<<GS_DIR<<"\nrun_completed = true\n";
    }
