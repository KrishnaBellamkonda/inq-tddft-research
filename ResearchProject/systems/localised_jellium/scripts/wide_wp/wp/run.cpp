// ============================================================================
// localised_jellium / scripts/qsp_phase5/wp / run.cpp
//
// PHASE-5 WP velocity sweep for the quantum stopping power S(E). IDENTICAL to the
// phase-4 WP run (90-box, slab well, two-sided sin² CAP, equidistant launch
// z=-23.75, GS shared_gs/slab_n82_L50x50x101_h0p40 reused) EXCEPT the drift momentum k0
// is now an env override `LJ_K0` so ONE build sweeps every velocity. Energy is a
// real-time-only quantity, so the same converged GS is valid for all k0.
//
//   v ∈ {1.3, 3.0, 4.0, 5.0, 6.0}  → E = ½k0²·HA ∈ {23, 122, 218, 340, 490} eV
//   τ ≈ 200/v (cap 200); the dispatcher sets LJ_N_STEPS = round(τ/dt).
//
// Env: LJ_OUT, LJ_CAP(1), LJ_K0(=Cfg::WP_KZ), LJ_N_STEPS, LJ_DT(0.04),
//      LJ_WRITE_EVERY, LJ_WF_EVERY, LJ_LAUNCH_Z(-23.75).
// ============================================================================
#include <inq/inq.hpp>
...
int main() {
    ...
    const std::string OUT    = "results/" + env_s("LJ_OUT", "p5_wp");
    const double DT_AU       = env_d("LJ_DT", 0.04);
    const int    N_STEPS     = env_i("LJ_N_STEPS", 1250);
    const int    WRITE_EVERY = env_i("LJ_WRITE_EVERY", 4);
    const int    WF_EVERY    = env_i("LJ_WF_EVERY", 4);
    const double LAUNCH_Z    = env_d("LJ_LAUNCH_Z", -26.5);
    const double K0          = env_d("LJ_K0", Cfg::WP_KZ);   // drift momentum override (sweep)
    const double E_DRIFT_EV  = 0.5 * K0 * K0 * localised_jellium::config::HA_TO_EV_111;
    // full-PBC, enlarged box: two-sided sin^2 CAP, region [+/-41.5, +/-55.5] (14 Bohr/side),
    // eta -1.0 Ha. mid = 48.5/111, width = 14.0/111.
    ...
    const std::string GS_DIR = env_s("LJ_GS_DIR",
        ...
        "shared_gs/slab_n82_L50x50x111_h0p40_pbc");
    if (!std::filesystem::exists(GS_DIR)) { std::cerr << "FATAL: GS missing: " << GS_DIR << "\n"; return 2; }

    std::cout << "\n=== qsp_phase5 wp (cap=" << (USE_CAP?"on":"off") << ", out=" << OUT << ") ===\n"
              << "  k0=" << K0 << " (E=" << E_DRIFT_EV << " eV)  N_STEPS=" << N_STEPS << " dt=" << DT_AU
              << " WRITE_EVERY=" << WRITE_EVERY << " launch_z=" << LAUNCH_Z << "\n";

    auto cell = systems::cell::orthorhombic(Cfg::LX_BOHR * 1.0_b, Cfg::LY_BOHR * 1.0_b, Cfg::LZ_BOHR * 1.0_b).periodic();  // full 3D PBC (periodic in z too)
    ...
    if (electrons.root()) {
        ...
        s << std::setprecision(12)
          << "run = localised_jellium/wide_wp/" << env_s("LJ_OUT","p5_wp") << "\n"
          << "engine = inq-study\n"
          << "projectile = wavepacket sigma " << Cfg::WP_SIGMA_BOHR << " E " << E_DRIFT_EV << " eV k0 " << K0 << "\n"
          << "boundary = periodicity 3 (full 3D PBC)\n"
          << "cap = " << (USE_CAP?"on (two-sided sin2, eta -1.0 Ha, 14 Bohr/side, region +/-41.5..+/-55.5, inner faces +/-41.5)":"off") << "\n"
