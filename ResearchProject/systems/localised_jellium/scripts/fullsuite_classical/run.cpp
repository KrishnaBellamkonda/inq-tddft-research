...
static const char* PROJ_PSEUDO =
    ...
    "shared/pseudopotentials/electron_gaussian_sigma0p35.upf";
...
int main() {
    ...
    const double LAUNCH_Z    = env_d("LJ_LAUNCH_Z", -15.5);
    const double V0          = Cfg::WP_K0;
    const double CAP_ETA = -0.5, CAP_MID = 0.425, CAP_WIDTH = 0.15;
    ...
    if (electrons.root()) {
        ...
        s << std::setprecision(12)
          << "run = localised_jellium/fullsuite_classical/" << env_s("LJ_OUT","p5_classical") << "\n"
          << "engine = inq-study\n"
          << "projectile = classical Gaussian-e ion (sigma_pot 0.35, mass m_e)\n"
          << "cap = " << (USE_CAP?"on (sin2 eta -0.5 mid +/-0.425 width 0.15)":"off") << "\n"
          << "cell_bohr = " << Cfg::L_BOHR << "^3  spacing = " << Cfg::SPACING_BOHR << "\n"
