...
#include <inqkit/jellium/projectile_background_energy.hpp>   // gaussian_density (unused now)
#include <inqkit/jellium/gaussian_potential.hpp>             // direct erf/r V_proj (ledger)
#include <inqkit/jellium/interaction_energies.hpp>           // compute_coulomb_direct (P/S/B)
...
int main(){
    ...
    // ---- two-sided sin^2 CAP (IDENTICAL to the WP quantum runs) ----
    // eta<0 absorbs; bands at +/-CAP_INNER..+/-(CAP_INNER+CAP_WIDTH), fractional cell coords
    // (perturbations::absorbing compares point_op.rvector()[2] in [-0.5,0.5)). Needs inq-study.
    const double CAP_ETA   = env_d("LJ_CAP_ETA",  -1.0);        // Ha
    const double CAP_INNER = env_d("LJ_CAP_INNER_BOHR", 30.0);  // inner edge of each band
    const double CAP_WIDTH = env_d("LJ_CAP_WIDTH_BOHR", 12.5);  // band width
    const double CAP_MID_FRAC   = (CAP_INNER + 0.5*CAP_WIDTH)/LZ; // 36.25/85 = 0.426470588235
    const double CAP_WIDTH_FRAC = CAP_WIDTH/LZ;                   // 12.5/85 = 0.147058823529
    ...
    // two-sided CAP: -z band at -MID_FRAC, +z band at +MID_FRAC (fractional cell coords)
    ...
    auto pert = perturbations::sum(perturbations::sum(perturbations::sum(bg_pert, proj_pert), cap_lo), cap_hi);
    ...
    rt.add([&](inqkit::StepContext const& ctx){
        ...
        // DIRECT-potential pairwise ledger: build the direct erf/r field V_proj ONCE and
        // use it for BOTH the projectile pairwise terms AND U_proj_bg. No charge density,
        // no Poisson, no periodic sheet -> e_ps positive & ->0, e_pp constant, no exit kink
        // (compute_coulomb_direct, interaction_energies.hpp). Slab terms stay Poisson-exact.
        ...
        const double Uprojbg = ct.e_pb;   // projectile-background (direct; ->0 as projectile exits)
    ...
    // ETRS (NOT Crank-Nicolson: CN renormalises each step and defeats CAP absorption).
    auto opts=options::real_time{}.num_steps(N_STEPS).dt(DT*1.0_atomictime).etrs();
    ...
    if(electrons.root()){ pj.close(); ix.close(); std::ofstream s(OUT+"/run_summary.txt");
        s<<std::setprecision(12)
         <<"run = classical_highdensity_sv/dyn_direct_cap (DIRECT erf/r potential Ehrenfest + two-sided CAP)\nengine = inq-study\n"
         ...
         <<"ledger = compute_coulomb_direct (e_pp analytic const, e_ps/e_pb from erf/r; no clip/sheet)\n"
         <<"propagator = etrs (CAP-correct; CN defeats absorption)\n"
         <<"cap = two-sided sin2  eta_ha = "<<CAP_ETA<<"  inner_bohr = +/-"<<CAP_INNER
         <<"  width_bohr = "<<CAP_WIDTH<<"  mid_frac = +/-"<<CAP_MID_FRAC<<"  width_frac = "<<CAP_WIDTH_FRAC<<"\n"
         <<"drive = ehrenfest\n"
