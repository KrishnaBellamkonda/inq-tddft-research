// ============================================================================
// classical_highdensity_sv / pilot / run.cpp   (Phase 3 pilot — RUN A)
//
// PRIMARY pilot: single-transit PERTURBATION Ehrenfest of a mass-1 Gaussian-charge
// electron projectile through the high-density localised jellium slab
// (r_s=4.18, N=100, 35x35x85, dx=0.5, periodicity 2). Cloned from
// scripts/classical_slab_stopping/run.cpp with two changes:
//
//   (1) config -> slab_n100_L35x35x85 (LX=LY=35, LZ=85, HALF=12.5, N=100,
//       EDGE_W=1.0, dx=0.5, periodicity 2, GS = shared_gs/slab_n100_...).
//   (2) FORCE: the FD projectile_force_z(phi_e) - projectile_force_z(phiplus) is
//       REPLACED by INQ's exact native analytic HF force (density-gradient form,
//       validated <0.1% vs native forces_stress):
//         Fz = projectile_force_analytic_z(electrons.density(), cell, center, SIGMA_POT)
//            - projectile_force_analytic_z(n_plus,             cell, center, SIGMA_POT)
//       i.e. drag from (electrons - background), each via the analytic operator.
//       The background analytic-force contribution is CONSTANT (n_+ and center-z
//       both change, but n_+ is a fixed field) — but center moves, so we recompute
//       it each step (cheap: one poisson + gradient of the constant n_+).
//
...
//   LJ_SAVE_EVERY(5) LJ_GS_DIR(REQUIRED) LJ_OUT(pilot) LJ_RESUME(0).
...
#include <inqkit/dynamics/projectile_force.hpp>              // analytic + FD force
...
int main(){
    ...
    const std::string GS_DIR=env_s("LJ_GS_DIR",""), OUT="results/"+env_s("LJ_OUT","pilot");
    ...
    std::cout<<std::setprecision(12)<<"\n=== pilot (perturbation Ehrenfest, ANALYTIC force) OUT="<<OUT
    ...
    // constant background fields (n_+ for the analytic force & U_proj_bg)
    ...
    rt.add([&](inqkit::StepContext const& ctx){
        ...
        // Ehrenfest: INQ-native ANALYTIC HF force from (electrons - background).
        // projectile_force_analytic is linear in the density field, so the drag
        // = F(n_e) - F(n_+); both evaluated at the SAME projectile center.
        const double Fz = inqkit::dynamics::projectile_force_analytic_z(electrons.density(), cell, center, SIGMA_POT)
                        - inqkit::dynamics::projectile_force_analytic_z(nplus,             cell, center, SIGMA_POT);
    ...
    if(electrons.root()){ pj.close(); ix.close(); std::ofstream s(OUT+"/run_summary.txt");
        s<<std::setprecision(12)
         <<"run = classical_highdensity_sv/pilot (perturbation Ehrenfest, analytic HF force)\nengine = inq-study\n"
         <<"representation = perturbation\nprojectile = moving Gaussian charge (velocity-Verlet)\n"
         <<"force = projectile_force_analytic (INQ-native density-gradient HF)\n"
         <<"drive = ehrenfest\n"
