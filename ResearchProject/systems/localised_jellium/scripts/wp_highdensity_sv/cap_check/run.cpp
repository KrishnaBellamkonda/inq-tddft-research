// ============================================================================
// localised_jellium / scripts/wp_highdensity_sv / cap_check / run.cpp
//
...
// Twin of campaign classical-highdensity-sv (docs/campaigns/localised_jellium/).
//
// WHAT IS IDENTICAL TO PRODUCTION
//   cell 35 x 35 x 85 Bohr, orthorhombic, periodicity(2), dx = 0.5
//   sigma_WP = 0.5 Bohr, launch z = -24, dt = 0.04
...
//   CAP_MID_FRAC   = 0.5 - WIDTH/2 = 0.426470588235  ( = 36.25 Bohr )
//   +z band: z in [ +30.0, +42.5 ] Bohr      (peak |eta| at +36.25)
//   -z band: z in [ -42.5, -30.0 ] Bohr      (peak |eta| at -36.25)
//
...
//
// CAVEAT THIS RUN WILL QUANTIFY: sigma_WP = 0.5 spreads at 1/(sqrt2 sigma) =
// 1.414 Bohr per a.u., so the packet's BACKWARD tail reaches the -z CAP inner
// edge (-30, only 6 Bohr behind the launch point) within a few a.u. Early norm
// loss is therefore EXPECTED and is a property of the matched width, not a bug.
// The notebook reports how much norm is gone by the time the packet reaches the
// slab region.
//
// Env: WPC_K0(2.0) WPC_ETA(-1.0) WPC_NSTEPS(1200) WPC_DT(0.04) WPC_H(0.50)
//      WPC_SIGMA(0.5) WPC_LAUNCH_Z(-24) WPC_SAVE_EVERY(10) WPC_OUT
// ============================================================================
#include <inq/inq.hpp>
...
int main() {
    ...
    // ---- production geometry (must match slab_n100_L35x35x85.hpp) ----------
    const double LX = 35.0, LY = 35.0, LZ = 85.0;
    // dx: 0.50 reproduces the CAP-geometry check at the classical grid; the
    // VACUUM CONTROLS for production run at the production grid (0.40) so their
    // CAP-only <p_z>(t) baseline is subtractable from the slab runs step for step.
    const double H  = env_d("WPC_H", 0.50);       // dx
    const int    PER = 2;                         // periodicity(2)

    // ---- wavepacket -------------------------------------------------------
    const double SIGMA    = env_d("WPC_SIGMA", 0.5);      // sigma_WP (sigma-wp-convention)
    const double K0       = env_d("WPC_K0", 2.0);         // = v (m = 1)
    const double LAUNCH_Z = env_d("WPC_LAUNCH_Z", -24.0);

    // ---- CAP --------------------------------------------------------------
    const double CAP_L    = env_d("WPC_CAP_L", 12.5);     // Bohr, per face
    const double ETA      = env_d("WPC_ETA", -1.0);       // Ha; <0 absorbs; 0 = control
    const double CAP_WIDTH_FRAC = CAP_L / LZ;                        // 0.147058823529
    const double CAP_MID_FRAC   = 0.5 - CAP_WIDTH_FRAC / 2.0;        // 0.426470588235
    const bool   CAP_ON   = (ETA != 0.0);

    // ---- propagation ------------------------------------------------------
    const double DT       = env_d("WPC_DT", 0.04);
    const int    N_STEPS  = env_i("WPC_NSTEPS", 1200);
    const int    SAVE_EVERY = env_i("WPC_SAVE_EVERY", 10);
    const std::string OUT = "results/" + env_s("WPC_OUT", (CAP_ON ? "cap" : "nocap"));

    const double E_eV     = 0.5 * K0 * K0 * HA_TO_EV;
    const double z_cap_in = LZ/2.0 - CAP_L;               // +30.0 Bohr
