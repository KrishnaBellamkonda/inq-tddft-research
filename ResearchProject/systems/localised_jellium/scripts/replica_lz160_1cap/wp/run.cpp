...
#include "../../../shared/configs/slab_n102_L25x25x160_w0p5_1cap.hpp"
...
using Cfg = localised_jellium::config::SlabN102_L25x25x160_w0p5_1cap;
...
int main() {
    ...
    const int    N_STEPS     = env_i("WP_N_STEPS", 8000);        // 100 a.u.
    ...
    // ONE-SIDED CAP: single 20-Bohr band at the +z end (z in [60,80]); replaces the
    // old two-sided cap_lo+cap_hi. Same TOTAL CAP length.
    perturbations::absorbing cap(CAP_ETA*1.0_Ha, Cfg::CAP_MID_FRAC, Cfg::CAP_WIDTH_FRAC);
    auto pert = perturbations::sum(bg_pert, cap);
