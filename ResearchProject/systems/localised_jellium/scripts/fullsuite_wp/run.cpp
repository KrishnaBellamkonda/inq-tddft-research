...
int main() {
    ...
    const bool   USE_CAP     = env_i("LJ_CAP", 0) != 0;
    const std::string OUT    = "results/" + env_s("LJ_OUT", "p3_wp");
    const double DT_AU       = env_d("LJ_DT", 0.02);
    const int    N_STEPS     = env_i("LJ_N_STEPS", USE_CAP ? 900 : 880);
    const int    WRITE_EVERY = env_i("LJ_WRITE_EVERY", 10);
    const int    WF_EVERY    = env_i("LJ_WF_EVERY", 10);
    const double LAUNCH_Z    = env_d("LJ_LAUNCH_Z", USE_CAP ? -15.5 : -23.0);
    const double CAP_ETA = -0.5, CAP_MID = 0.425, CAP_WIDTH = 0.15;  // 7.5 Bohr/side
