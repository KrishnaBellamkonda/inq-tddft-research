// ============================================================================
// localised_jellium / scripts/extkin_plateau_E100 / wp / run.cpp
//
// WP projectile (σ=1.5, E=100 eV) through the N=92 / r_s=4.0 / 20.13-Bohr slab
// in the 35×35×120 box, TWO-SIDED sin² CAP (15 Bohr, η=−1.0) — the first
// jellium run carrying the norm-division fix IN-RUN:
// inqkit::observables::OrbitalKineticStats records the BARE per-orbital kinetic
// + norm for ALL states every step, so the corrected extensive total
//   E_corr = total − kinetic + kin_bare_total
// (and hence E_plateau) comes straight out of the run, with the per-step
// identity kin_normdiv_total == energies.csv:kinetic as a built-in check.
// Design + decision log: docs/plans/norm-corrected-stopping-power.md
// "Run design (2026-07-29)". Vacuum validation of the observable:
// docs/notes/inq-energy-normalization-error.md "IN-RUN FIX VALIDATED".
//
// ⚠ dt=0.04 default is the FIRST CAP run at this dt (H·dt=0.79; user skipped
// the vacuum gate) — check absorbed-norm sanity in analysis.
//
// Engine: inq-study (stock inq cannot compile a real-time absorbing run).
//
// Env: WP_CAP_ETA(-1.0; 0=no CAP) WP_OUT WP_GS_DIR WP_N_STEPS(1500)
//      WP_DT(0.04) WP_WF_EVERY(100) WP_DENS_EVERY(10) WP_CKPT_EVERY(200)
//      WP_EXTKIN(1) WP_EXTKIN_EVERY(1) WP_RESUME(0) WP_COMPILE_PROBE=1
// ============================================================================
#include <inq/inq.hpp>
...
#include "../../../shared/configs/slab_n92_L35x35x120_w0p5.hpp"
...
using Cfg = localised_jellium::config::SlabN92_L35x35x120_w0p5;
...
int main() {
    ...
    const double CAP_ETA     = env_d("WP_CAP_ETA", Cfg::CAP_ETA_HA);
    const bool   CAP_ON      = (CAP_ETA != 0.0);
    const std::string OUT    = "results/" + env_s("WP_OUT", (CAP_ON? "cap":"nocap"));
    const double DT_AU       = env_d("WP_DT", Cfg::DT_AU_DEFAULT);
    const int    N_STEPS     = env_i("WP_N_STEPS", Cfg::N_STEPS_DEFAULT);
    const int    WF_EVERY    = env_i("WP_WF_EVERY", 100);
    const int    DENS_EVERY  = env_i("WP_DENS_EVERY", 10);
    const int    CKPT_EVERY  = env_i("WP_CKPT_EVERY", 200);
    const int    EXTKIN_ON   = env_i("WP_EXTKIN", 1);
    const int    EXTKIN_EVERY= env_i("WP_EXTKIN_EVERY", 1);
    ...
    std::cout << std::setprecision(6)
              << "\n=== extkin_plateau_E100 WP (out=" << OUT << ") ===\n"
              << "  CAP " << (CAP_ON?"ON (two-sided)":"OFF") << " eta=" << CAP_ETA
              << " L=" << Cfg::CAP_L_BOHR << "/side\n"
              << "  sigma=" << Cfg::WP_SIGMA_BOHR << " E=" << Cfg::WP_EKIN_EV << " eV k0=" << Cfg::WP_K0 << "\n"
              << "  launch_z=" << Cfg::WP_CZ_BOHR << " N_STEPS=" << N_STEPS << " dt=" << DT_AU
              << " extkin=" << EXTKIN_ON << "/" << EXTKIN_EVERY << " (start=" << START << ")\n";
    ...
    obs_::DensityDelta density_delta(OUT + "/raw/vti/density_delta", OUT + "/raw/vti/density_delta_coarse",
        {.emit_raw_vti=false, .emit_coarse_vti=true, .compute_l2=true, .coarse_bin_bohr=3.0});
    ...
    // ----- the IN-RUN norm-division fix: bare per-orbital kinetic + norms ----
    std::optional<obs_::OrbitalKineticStats> extkin;
    if (EXTKIN_ON)
        extkin.emplace(OUT + "/raw/observables/orbital_kinetic_stats" + SEG + ".csv",
                       obs_::OrbitalKineticStatsConfig{.write_every = EXTKIN_EVERY});
    ...
    const double prop_wall = std::chrono::duration<double>(std::chrono::steady_clock::now() - tp0).count();
    en.close();

    // final checkpoint (rule final-timestep-checkpoint; resume becomes a no-op)
    electrons.save(RT_CKPT_DIR);
    if (electrons.root()) {
        std::ofstream st(RT_STATE_TXT, std::ios::trunc);
    ...
    if (electrons.root()) {
        ...
        s << std::setprecision(12)
          << "run = localised_jellium/extkin_plateau_E100/" << env_s("WP_OUT",(CAP_ON?"cap":"nocap")) << "\n"
          << "engine = inq-study\ntheory = lda\npropagator = etrs (default)\n"
          << "cap = " << (CAP_ON?"on (two-sided)":"off") << "  cap_eta_ha = " << CAP_ETA
          << "  cap_L_bohr = " << Cfg::CAP_L_BOHR << "/side  cap_mid_frac = +/-" << Cfg::CAP_MID_FRAC << "\n"
          ...
          << "extkin = " << (EXTKIN_ON?"on":"off") << "  extkin_every = " << EXTKIN_EVERY << "\n"
          << "resume = " << (RESUME?"true":"false") << "  start_step = " << START
          << "  segment_suffix = " << (SEG.empty()?"(none)":SEG) << "\n"
          << "propagate_wall_s = " << prop_wall
          << "  per_step_ms = " << (1000.0*prop_wall/std::max(1, N_STEPS-START)) << "\n"
