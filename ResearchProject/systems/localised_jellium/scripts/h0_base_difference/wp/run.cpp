// ============================================================================
// localised_jellium / scripts/h0_base_difference / wp / run.cpp
//
// H0 (base WP-vs-classical E_total(0) gap). Loads the 50x50x120 slab GS, injects
// a STATIONARY wavepacket (k0 = 0, sigma_WP = 0.5) at z = LJ_LAUNCH_Z, re-applies
// the background well, and propagates a few short steps to read E_total(t=0) from
// the standard observables writer (which records step 0). No CAP (closed box,
// only ~2 steps). The excess E_total(0) - E_GS - <T_WP> is computed in the H0
// notebook; <T_WP> here is the zero-point 3/(4 sigma^2) (k0 = 0).
//
// Env: LJ_OUT(h0_wp_r4) LJ_LAUNCH_Z(-16.5) LJ_N_STEPS(2) LJ_DT(0.01) LJ_K0(0).
...
int main() {
    ...
    std::cout << "\n=== H0 wp (out=" << OUT << ") launch_z=" << LAUNCH_Z
              << " k0=" << K0 << " sigma=" << Cfg::WP_SIGMA_BOHR
    ...
    // ----- WP injection (stationary for H0) ------------------------------
    ...
    const double zero_point_ha = 3.0 / (4.0 * Cfg::WP_SIGMA_BOHR * Cfg::WP_SIGMA_BOHR);
    double wall = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
    if (electrons.root()) {
        std::ofstream s(OUT + "/run_summary.txt");
        s << std::setprecision(12)
          << "run = localised_jellium/h0_base_difference/wp/" << env_s("LJ_OUT","h0_wp_r4") << "\n"
          << "engine = inq-study\n"
          << "projectile = wavepacket sigma_WP " << Cfg::WP_SIGMA_BOHR << " k0 " << K0 << " (stationary)\n"
          ...
          << "wp_norm_after = " << report.norm_after << "  wp_state_index = " << wp_idx << "\n"
          << "zero_point_T_WP_ha = " << zero_point_ha << "  (= 3/(4 sigma^2), k0=0)\n"
