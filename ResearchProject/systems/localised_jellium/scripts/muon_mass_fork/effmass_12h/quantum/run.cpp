// ============================================================================
// localised_jellium / muon_mass_fork / effmass_12h / quantum / run.cpp
//
...
// Loads shared_gs/slab_n42_L36x36x80_dx0p40. Adapted from effmass_pair/quantum.
//
...
#include "../../../../shared/configs/slab_n42_L36x36x80.hpp"
#include <chrono>
#include <cstdlib>
#include <filesystem>
#include <fstream>
...
using Cfg = localised_jellium::config::SlabN42_L36x36x80;
...
int main() {
	...
	const double K0           = env_d("EM_K0", 6.7933);
	const double INV_MASS     = env_d("EM_INV_MASS", 0.39907);   // 1/2.506
	const double LAUNCH_Z     = env_d("EM_LAUNCH_Z", -16.389);
	const double DT_AU        = env_d("EM_DT", 0.05);
	const int    N_STEPS      = env_i("EM_N_STEPS", 846);
	const int    WRITE_EVERY  = env_i("EM_WRITE_EVERY", 20);
	const int    CKPT_EVERY   = env_i("EM_CKPT_EVERY", 100);
	...
	const std::string GS_DIR  = env_s("EM_GS_DIR",
		...
		"shared_gs/slab_n42_L36x36x80_dx0p40");
	...
	const double CAP_ETA = -0.7, CAP_MID = 32.0/80.0, CAP_WIDTH = 8.0/80.0;
	...
	if (electrons.root())
		std::cout << std::setprecision(8)
			<< "\n=== effmass_12h QUANTUM out=" << OUT << (RESUME?"  [RESUME]":"  [FRESH]") << " ===\n"
