// ============================================================================
// localised_jellium / muon_mass_fork / effmass_sigma1 / quantum / run.cpp
//
// CONCENTRATED sigma_WP=1 CHIRPED-FOCUS wavepacket through the r_s=5.68 slab
// (LEAN re-run, plan: docs/plans/effmass-sigma1-lean-rerun.md). m_eff=2.10,
// v=2.711 a.u. (S(v) anchor), k0=5.693, dx=0.33333 (chirp Nyquist requirement),
// dt=0.04, N=900 steps (3.9x traversal) on the density-matched 40x40x80 (N=52)
// slab. focus_z(4.0, 2.10) puts the WP waist (sigma_rho_z=0.707) at the slab
// face. Gaussian WP injected into the last extra state with tuned inverse_mass
// (the inq-study mass fork).
// Loads shared_gs/slab_n52_L40x40x80_dx0p333. Adapted from effmass_pair/quantum.
//
...
int main() {
	...
	const double INV_MASS     = env_d("EM_INV_MASS", 0.476190);   // 1/2.506
	...
	const int    CKPT_EVERY   = env_i("EM_CKPT_EVERY", 300);
	...
	// CAP for Lz=80: default region [25,40] each side — centre |z|=32.5, full width
	// 15 Bohr (reflectivity-tuned), inner face ±25 (packet 3sigma front at slab exit
	// = +22 stays clear). ENV-OVERRIDABLE for the CAP-parameter study (2026-07-11):
	// EM_CAP_ETA (Ha), EM_CAP_CENTER_BOHR (|z| of centre), EM_CAP_WIDTH_BOHR (full
	// width). Defaults reproduce the original run bit-for-bit.
	...
	if (RESUME) {
		...
		const double FOCUS_DIST = env_d("EM_FOCUS_DIST", 4.0);  // launch -> slab face; waist forms there
		auto wp = inqkit::WavePacket{}
		              .center(0.0, 0.0, LAUNCH_Z)
		              .sigma(SIGMA_WP)
		              .k0(0.0, 0.0, K0)
		              .focus_z(FOCUS_DIST, 1.0/INV_MASS)   // chirped focusing launch (validated; Nyquist-safe at dx=0.333)
	...
	if (electrons.root())
		std::cout << std::setprecision(8)
			<< "\n=== effmass_sigma1 QUANTUM out=" << OUT << (RESUME?"  [RESUME]":"  [FRESH]") << " ===\n"
	...
	if (electrons.root()) {
		...
		s << std::setprecision(12)
		  << "run = localised_jellium/muon_mass_fork/effmass_sigma1/quantum\n"
