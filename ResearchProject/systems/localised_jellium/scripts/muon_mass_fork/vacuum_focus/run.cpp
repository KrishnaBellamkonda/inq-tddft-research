...
int main() {
    ...
    const std::string OUT     = "results/" + env_s("WP_OUT", "vacfocus");
    ...
    auto wp = inqkit::WavePacket{}
                  ...
                  .sigma(SIGMA_WP)
                  .k0(0.0, 0.0, K0);
    // FOCUS TEST: converging launch so the waist forms WP_FOCUS_DIST ahead.
    const double FOCUS_DIST = env_d("WP_FOCUS_DIST", 0.0);
    if (FOCUS_DIST > 0.0) { wp.focus_z(FOCUS_DIST, 1.0/INV_MASS);
        if (electrons.root()) std::cout << "  FOCUS ON: dist=" << FOCUS_DIST
            << " mass=" << 1.0/INV_MASS << " -> waist at z=+" << FOCUS_DIST
            << " at t=" << FOCUS_DIST/(K0*INV_MASS) << " au\n"; }
