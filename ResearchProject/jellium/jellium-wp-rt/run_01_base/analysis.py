"""analysis.py — jellium run_01_base (N=38, 200 eV, σ=0.53 Å, +z)."""
...
if total_dir.exists() and jell_dir.exists() and wp_dir.exists():
    ...
    if t_files and j_files and w_files:
        ...
        rho_max = ft.array.max()
        ratio = diff / max(rho_max, 1e-30)
...
# ── 5. VTI conversion (50 frames × 3 series) ─────────────────────────────────
...
for series_name in ("density_rt_total", "density_rt_jellium", "density_rt_wp"):
    ...
    out_dir = VISDIR / "vti" / series_name
    convert_real_series_to_vti(series_obj, out_dir)
...
if screens_dir.exists():
    ...
    if len(dat_files) == N_SCREENS:
        ...
        print(f"  Saved: {len(dat_files)} individual PNGs → screens_individual/")
    else:
...
if index_csv.exists():
    ...
    if index_rows:
        ...
        if first_mat is not None:
            ...
            print("  Could not read first overlap matrix")
    else:
        print("  index.csv is empty")
else:
    print("  Skip: results/overlap/index.csv not found")

# ── 10. ParaView 3D renders (low priority) ────────────────────────────────────
