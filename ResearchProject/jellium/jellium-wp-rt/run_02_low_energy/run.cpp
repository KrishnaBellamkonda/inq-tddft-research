// jellium-wp-rt / run_02_low_energy: N=40 jellium, 50 eV, sigma=0.53A, +z
#include <inq/inq.hpp>
...
static constexpr int    N_STEPS     = 834; // Half the momentum and hence twice the number of steps
...
int main() {
    std::cout << "\n=== jellium run_02_low_energy: N=40, 50 eV, sigma=0.53A, +z ===\n";
    auto cell = systems::cell::cubic(L_BOHR*1.0_b).periodic();
    auto ions = systems::ions(cell);
    auto electrons = systems::electrons(ions,
        options::electrons{}.spacing(0.50*1.0_b).extra_electrons(38).extra_states(3)
