// run_05_transverse_x: free WP, 200 eV, sigma=0.53A, +x direction
// Center placed near x=0 wall (x=5*sigma) so WP traverses cell in +x
#include <inq/inq.hpp>
...
static const double WP_CX = 5.0 * WP_SIGMA_BOHR;  // near x=0 wall
static const double WP_CY = LY_BOHR / 2.0;
static const double WP_CZ = LZ_BOHR / 2.0;
...
int main() {
    std::cout << "\n=== run_05_transverse_x: free WP, 200 eV, sigma=0.53A, +x ===\n";
    ...
    auto wp = inqkit::WavePacket{}.center(WP_CX, WP_CY, WP_CZ).sigma(WP_SIGMA_BOHR).k0(WP_K0, 0.0, 0.0);
