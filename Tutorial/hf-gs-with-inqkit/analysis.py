"""
hf-gs-with-inqkit: analysis.py
Validates the inqkit write → inqview read → visualise pipeline for HF molecule.

Run with: /local/data/public/skcb2/pyenv/versions/3.10.19/envs/quantum-wave-packet/bin/python3 analysis.py
Expected norms: N_elec ≈ 8, HOMO density norm ≈ 1.0, HOMO |ψ|² norm ≈ 1.0
"""
...
from inqview.paraview import ParaViewPipeline, VolumeRenderSpec, AnimationSpec, AtomSpec
...
print(f"[density]   N_electrons = {n_elec:.3f}   (expect ≈ 8.0)")

# ── 3. HOMO orbital density (|ψ_3|²) ────────────────────────────────────────
...
FAIL = "\u2717"
print("\n=== Validation summary ===")
print(f"  {PASS if abs(n_elec - 8.0) < 0.5 else FAIL}  N_electrons = {n_elec:.3f}  (expect 8 ± 0.5)")
...
def _save_slices(arr, label, filename):
    ...
    fig.suptitle(f"HF molecule — {label}")
    ...
    axes[2].set_title("x-y slice (z=mid)")
...
# ── 8. ParaView renders ──────────────────────────────────────────────────────
# HF atoms centred at (L/2, L/2, L/2) = (8, 8, 8) bohr in a 16-bohr cubic cell.
# Bond length 0.917 Å = 1.733 bohr; H below centre, F above.
_HF_ATOMS = AtomSpec(
    positions=[[8.000, 8.000, 7.134], [8.000, 8.000, 8.867]],
    symbols=["H", "F"],
    radius_scale=0.4,
)
